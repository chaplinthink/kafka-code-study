...
public class MemoryRecords implements Records {
    ...
    private final Compressor compressor; //压缩器，对消息数据进行压缩，将压缩后的数据输出到buffer

    // the write limit for writable buffer, which may be smaller than the buffer capacity
    private final int writeLimit; //记录了buffer字段最多可以写入多少个字节的数据
    ...
    private ByteBuffer buffer; //用来保存消息数据的java NIO ByteBuffer

    // indicate if the memory records is writable or not (i.e. used for appends or read-only)
    private boolean writable; //此MemoryRecords对象是只读模式还是可写模式。在MemoryRecords发送前，会将其设置成只读模式

    // Construct a writable memory records 私有的构造
    ...
    public void append(long offset, Record record) {
        if (!writable)  //判断MemoryRecords是否为可写模式
            throw new IllegalStateException("Memory records is not writable");

        int size = record.size();
        compressor.putLong(offset);    //调用compressor.put*（）方法将消息数据写入ByteBuffer中
     ...
     */ //此方法是估算的，通过此方法判断，也可能导致ByteBuffer出现扩容的现象
    public boolean hasRoomFor(byte[] key, byte[] value) {
        ...
        return this.compressor.numRecordsWritten() == 0 ?
            ...
            this.writeLimit >= this.compressor.estimatedBytesWritten() + Records.LOG_OVERHEAD + Record.recordSize(key, value); //根据Compressor 估算的已写字节数，估计MemoryRecords剩余空间足够写入指定的数据
    }
     ...
     */  //ByteBuffer出现扩容的现象，会将MemoryRecords.buffer指向扩容后的ByteBuffer对象
    public void close() {
        if (writable) {
    ...
    public int sizeInBytes() {
        if (writable) {  //可写的 返回 ByteBufferOutputStream.buffer 字段的大小
            return compressor.buffer().position();
        } else {
            return buffer.limit(); //只读 返回MemoryRecords.buffer的大小
        }
    }
