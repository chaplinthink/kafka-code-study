...
public class Compressor {
    ...
    public Compressor(ByteBuffer buffer, CompressionType type) {
        this.type = type; //从KafkaProducer传递过来的压缩类型  KafkaProducer.compressionType
        ...
        // create the stream  装饰器模式
        bufferStream = new ByteBufferOutputStream(buffer); //ByteBufferOutputStream继承了java.io.OutputStream，封装了ByteBuffer当写入数据超出ByteBuffer容量时，ByteBufferOutputStream会进行自动扩容
        appendStream = wrapForOutput(bufferStream, type, COMPRESSION_DEFAULT_BUFFER_SIZE); //对bufferStream进行装饰，为其添加了压缩的功能. 根据压缩类型创建合适的压缩流
    }
    ...
    //估计已经写入的（压缩后）字节数，在判断MemoryRecords是否写满的逻辑会使用
    public long estimatedBytesWritten() {
        if (type == CompressionType.NONE) {
            return bufferStream.buffer().position();
        } else {
            // estimate the written bytes to the underlying byte buffer based on uncompressed written bytes
            return (long) (writtenUncompressed * TYPE_TO_RATE[type.id] * COMPRESSION_RATE_ESTIMATION_FACTOR); //写入的未压缩数据的字节数、指定压缩方式的压缩率、估算因子
        }
    }
    ...
    static public DataOutputStream wrapForOutput(ByteBufferOutputStream buffer, CompressionType type, int bufferSize) {
        try {
            switch (type) { //根据不同的压缩类型创建不同的压缩流
                case NONE: //不压缩的方式
                    return new DataOutputStream(buffer);
                case GZIP: //使用GZIP的压缩方式
                    return new DataOutputStream(new GZIPOutputStream(buffer, bufferSize)); //JDK自带 GZIPOutputStream
                case SNAPPY://使用SNAPPY的压缩方式
                    try {
                        OutputStream stream = (OutputStream) snappyOutputStreamSupplier.get().newInstance(buffer, bufferSize); //SNAPPY需要引入额外的依赖包，为了在不使用SNAPPY的时候，减少依赖包，这里使用反射的方式创建
    ...
    static public DataInputStream wrapForInput(ByteBufferInputStream buffer, CompressionType type, byte messageVersion) {
        try {
            switch (type) {
                ...
                case SNAPPY:
                    try {
                        InputStream stream = (InputStream) snappyInputStreamSupplier.get().newInstance(buffer); //通过反射的方式创建
