...
class ControllerBrokerRequestBatch(controller: KafkaController) extends  Logging {
  ...
  def addUpdateMetadataRequestForBrokers(brokerIds: Seq[Int],
    ...
    def updateMetadataRequestMapFor(partition: TopicAndPartition, beingDeleted: Boolean) {
      val leaderIsrAndControllerEpochOpt = controllerContext.partitionLeadershipInfo.get(partition) //获取该分区的leader，ISR,ControllerEpoch信息
      leaderIsrAndControllerEpochOpt match {
        case Some(leaderIsrAndControllerEpoch) =>
          val replicas = controllerContext.partitionReplicaAssignment(partition).toSet //获取该分区的AR集合
          val partitionStateInfo = if (beingDeleted) { //TODO：为什么要根据beingDeleted值来判断
          ...
          brokerIds.filter(b => b >= 0).foreach { brokerId =>
            updateMetadataRequestMap.getOrElseUpdate(brokerId, mutable.Map.empty[TopicPartition, PartitionStateInfo]) //如果brokerId获取不到值，对应值就是mutable.Map.empty[TopicPartition, PartitionStateInfo]
            updateMetadataRequestMap(brokerId).put(new TopicPartition(partition.topic, partition.partition), partitionStateInfo)   //如果key：brokerId 可以获取到值，其实就是向上一个map的value中put数据
          }
        case None =>
