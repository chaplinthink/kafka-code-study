...
object AclCommand {
  ...
  class AclCommandOptions(args: Array[String]) {
    ...
    val usernameOpt =  parser.accepts("username","REQUIRED: properties required to check the authorized user to update zookeeper rights")
      .withRequiredArg
      .describedAs("username")
      .ofType(classOf[String])

    val passwordOpt =  parser.accepts("password","REQUIRED: properties required to check the authorized user to update zookeeper rights")
      .withRequiredArg
    ...
    def checkArgs() {
      CommandLineUtils.checkRequiredArgs(parser, options, authorizerPropertiesOpt,usernameOpt,passwordOpt)

      CommandLineUtils.checkUsernamePasswordArgs(parser, options.valueOf(usernameOpt), options.valueOf(passwordOpt))
