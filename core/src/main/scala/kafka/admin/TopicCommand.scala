...
object TopicCommand extends Logging {
  ...
  class TopicCommandOptions(args: Array[String]) {
    ...
    val usernameOpt =  parser.accepts("username","REQUIRED: properties required to check user who has rights to do the action")
      .withRequiredArg
      .describedAs("username")
      .ofType(classOf[String])

    val passwordOpt =  parser.accepts("password","REQUIRED: properties required to check user who has rights to do the action")
      .withRequiredArg
    ...
    def checkArgs() {
      // check required args
      CommandLineUtils.checkRequiredArgs(parser, options, zkConnectOpt, usernameOpt, passwordOpt)
