...
class ReplicaManager(val config: KafkaConfig,
   ...
   */
  def readFromLocalLog(fetchOnlyFromLeader: Boolean, //是否读去leader副本的消息，只有处理来自debug模式下的消费者的请求该参数才会为false,正常运行的模式下始终为true
                       readOnlyCommitted: Boolean, // 是否读取完成提交的消息（即HW之前的消息）
                       readPartitionInfo: Map[TopicAndPartition, PartitionFetchInfo]): Map[TopicAndPartition, LogReadResult] = {  // 第三方参数，每个分区开始读写的起始位置和最大字节数
    ...
    readPartitionInfo.map { case (TopicAndPartition(topic, partition), PartitionFetchInfo(offset, fetchSize)) =>
      ...
      val partitionDataAndOffsetInfo =
        try {
          ...
          //获取要读取消息的副本，判断是否必须是leader副本
          // decide whether to only fetch from leader
          val localReplica = if (fetchOnlyFromLeader)
            getLeaderReplicaIfLocal(topic, partition)
          else
            getReplicaOrException(topic, partition)
           //确定读取offset消息的上限，根据readOnlyCommitted是否为HW
          ...
          val logReadInfo = localReplica.log match { //logReadInfo是FetchDataInfo，其中包含LogOffsetMetadata 和 MessageSet
            case Some(log) =>
              log.read(offset, fetchSize, maxOffsetOpt) //从log中读取消息
            case None =>
