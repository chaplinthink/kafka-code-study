...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleOffsetCommitRequest(request: RequestChannel.Request) {
    ...
    /**
   * @param  isCheck   need do check ?
   *                   true  do authorizer  else not do
   **/
  private def authorize(session: Session, operation: Operation, resource: Resource, isCheck: Boolean): Boolean = 
    isCheck || authorizer.map(_.authorize(session, operation, resource)).getOrElse(true)
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    var clientId = fetchRequest.clientId;
    //if(clientId != null){
    //  val index = clientId.lastIndexOf("_")
    //  if(clientId.contains("_") && clientId.length - index <= 3 ){
    //    clientId = clientId.substring(0,index);
    //  }
    //} 
    val session = Session(new KafkaPrincipal(KafkaPrincipal.USER_TYPE , clientId ), request.session.clientAddress)
    val (authorizedRequestInfo, unauthorizedRequestInfo) = fetchRequest.requestInfo.partition {
      case (topicAndPartition, _) => authorize(session, Read, new Resource(Topic, topicAndPartition.topic), fetchRequest.isFromFollower)
    }
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request) {
    ...
    var clientId = request.header.clientId();
    //if(clientId != null){
    //  val index = clientId.lastIndexOf("_")
    //  if(clientId.contains("_") && clientId.length - index <= 3 ){
    //    clientId = clientId.substring(0,index);
    ...
    var (authorizedTopics, unauthorizedTopics) =
      topics.partition(topic => authorize(session, Describe, new Resource(Topic, topic),false))
