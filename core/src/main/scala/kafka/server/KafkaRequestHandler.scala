    ...
    while(true) {
      try {
        ...
        while (req == null) {
          ...
          req = requestChannel.receiveRequest(300) //从RequestChannel.requestQueue获取请求，通过requestQueue.poll()实现
          val idleTime = SystemTime.nanoseconds - startSelectTime
          aggregateIdleMeter.mark(idleTime / totalHandlerThreads) //统计监控指标
        }
        //读取到RequestChannel.AllDone 请求，KafkaRequestHandler线程结束
        ...
        apis.handle(req)//KafkaApis类中实现了处理请求的逻辑，KafkaApi还负责将响应写回对应的requestChannel.sendResponse
      } catch {
