...
class SimpleAclAuthorizer extends Authorizer with Logging {
  ...
  val aclAuth = new util.ArrayList[ACL]()
  aclAuth.add(ZkUtils.AuthorizedAcl)
  /**
  ...
  override def configure(javaConfigs: util.Map[String, _]) {
    ...
    zkUtils.makeSurePersistentPathExists(SimpleAclAuthorizer.AclZkPath, aclAuth)

    loadCache()

    zkUtils.makeSurePersistentPathExists(SimpleAclAuthorizer.AclChangedZkPath, aclAuth)
  ...
  private def aclMatch(session: Session, operations: Operation, resource: Resource, principal: KafkaPrincipal, host: String, permissionType: PermissionType, acls: Set[Acl]): Boolean = {
    acls.find ( acl =>
      acl.permissionType == permissionType
        && (acl.principal == principal ||session.principal.getName.contains(acl.principal.getName)|| acl.principal == Acl.WildCardPrincipal)
  ...
  private def logAuditMessage(principal: KafkaPrincipal, authorized: Boolean, operation: Operation, resource: Resource, host: String) {
    ...
    if ("Denied".equals(permissionType)) {
      authorizerLogger.warn(s"Principal = $principal is $permissionType Operation = $operation from host = $host on resource = $resource")
    }
  }
    ...
    try {
      ...
      case e: ZkNoNodeException =>
        try {
          ...
          zkUtils.createPersistentPath(path, data, aclAuth)
          (true, 0)
        } catch {
  ...
  private def updateAclChangedFlag(resource: Resource) {
    zkUtils.createSequentialPersistentPath(SimpleAclAuthorizer.AclChangedZkPath + "/" + SimpleAclAuthorizer.AclChangedPrefix, resource.toString, aclAuth)
  }
