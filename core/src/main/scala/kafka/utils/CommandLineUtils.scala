...
object CommandLineUtils extends Logging {
  ...
  */
 def checkUsernamePasswordArgs(parser: OptionParser,username:String,password:String){
    val generateDigest = DigestAuthenticationProvider.generateDigest(username.trim+":"+password.trim)
    if(!generateDigest.equals(ZkUtils.userInfo)){
      printUsageAndDie(parser, "username and password is wrong ")
    }
  }
