...
object ZkUtils {
  ...
  val userInfo = "kafka:xxxxxx"
  val AuthorizedAcl = new ACL(Perms.ALL, new Id("digest", userInfo))
  ...
  def createZkClient(zkUrl: String, sessionTimeout: Int, connectionTimeout: Int): ZkClient = {
    ...
    zkClient.addAuthInfo("digest","kafka:xxxxx".getBytes)
    zkClient
  }
  ...
  def DefaultAcls(isSecure: Boolean): java.util.List[ACL] ={
    ...
    if (isSecure) {
      ...
      //list.add(new ACL(Perms.ALL, new Id("ip", "10.168.99.154")))
      list.add(AuthorizedAcl)
      list.addAll(ZooDefs.Ids.READ_ACL_UNSAFE)
  }
    list
  }
