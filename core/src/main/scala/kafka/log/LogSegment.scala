...
class LogSegment(val log: FileMessageSet, //用于操作对应日志文件的FileMessageSet对象
   ...
   */
  @threadsafe //offset与物理地址之间的转换
  private[log] def translateOffset(offset: Long, startingFilePosition: Int = 0): OffsetPosition = {
    val mapping = index.lookup(offset) //对索引文件查找
    log.searchFor(offset, max(mapping.position, startingFilePosition)) //通过FileMessageSet完成对日志文件的查找
  }
   ...
   */
  @threadsafe //参数1：指定读取的起始消息的offset  2:指定读取结束的offset，可为空 3：读取的最大字节数  4： 读取的最大物理地址，默认是日志文件的大小
  def read(startOffset: Long, maxOffset: Option[Long], maxSize: Int, maxPosition: Long = size): FetchDataInfo = {
    if(maxSize < 0)
      throw new IllegalArgumentException("Invalid max size for log read (%d)".format(maxSize))

    val logSize = log.sizeInBytes // this may change, need to save a consistent copy     日志文件的长度
    val startPosition = translateOffset(startOffset) //将startOffset转换为物理地址
