 ...
 */
@nonthreadsafe //管理日志文件，对应磁盘上的一个真正的日志文件
class FileMessageSet private[kafka](@volatile var file: File,
  ...
  def searchFor(targetOffset: Long, startingPosition: Int): OffsetPosition = {
    ...
    val buffer = ByteBuffer.allocate(MessageSet.LogOverhead) //8字节的offset值与4字节的message data的大小组成了一个LogOverhead，message data保存了消息的数据
    val size = sizeInBytes() //当前FileMessageSet的大小，单位是字节
    while(position + MessageSet.LogOverhead < size) { //从position开始逐条消息遍历
      buffer.rewind()  //重置ByteBuffer的position指针，准备读入数据
      channel.read(buffer, position)  //读取LogOverhead.这里会确保startingPosition位于一个消息的开头，否则读取到的并不是LogOverhead
      if(buffer.hasRemaining) //没有读取到12字节的LogOverhead,抛出异常
