...
class Log(val dir: File,
  ...
  def read(startOffset: Long, maxLength: Int, maxOffset: Option[Long] = None): FetchDataInfo = {
    ...
    val currentNextOffsetMetadata = nextOffsetMetadata //把nextOffsetMetadata变成局部变量，避免线程安全的问题
    val next = currentNextOffsetMetadata.messageOffset
    if(startOffset == next)
      return FetchDataInfo(currentNextOffsetMetadata, MessageSet.Empty)

    var entry = segments.floorEntry(startOffset) //查找baseoffset小于startoffset且baseoffset最大的LogSegment
    ...
    while(entry != null) {
      ...
      val fetchInfo = entry.getValue.read(startOffset, maxOffset, maxLength, maxPosition) //调用LogSegment.read()方法读取消息
      if(fetchInfo == null) {
